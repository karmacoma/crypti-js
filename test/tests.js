		...
		describe("delegate.js", function () {
			...
			describe("#createDelegate", function () {
				...
				describe("returned delegate", function () {
					...
					var secondKeys = crypti.crypto.getKeys("secret 2");
					...
					it("should be signed correctly", function () {
						var result = crypti.crypto.verify(dlg, keys.publicKey);
						(result).should.be.ok;
					});

					it("should be second signed correctly", function () {
						var result = crypti.crypto.verifySecondSignature(dlg, secondKeys.publicKey);
						(result).should.be.ok;
					});
					...
					it("should be second signed not correctly right now", function () {
						dlg.amount = 100;
						var result = crypti.crypto.verify(dlg, secondKeys.publicKey);
						(result).should.be.not.ok;
					});
		...
		describe("vote.js", function () {
			...
			describe("#createVote", function () {
				var createVote = vote.createVote,
					vt = null,
					publicKey = crypti.crypto.getKeys("secret").publicKey,
				...
				describe("created vote", function () {
					...
					it("should have recipientId string equal to sender", function () {
						(vt).should.have.property('recipientId').and.be.type('string').and.equal(crypti.crypto.getAddress(publicKey))
					});
					...
					it("should be signed correctly", function () {
						var result = crypti.crypto.verify(vt);
						(result).should.be.ok;
					});

					it("should be signed second signature correctly", function () {
						var result = crypti.crypto.verifySecondSignature(vt, crypti.crypto.getKeys("second secret").publicKey);
						(result).should.be.ok;
					});
